...
{
    /// <summary>
    /// Represent a <see href="https://developer.xurrent.com/graphql/object/riskseverity/">RiskSeverity</see> in Xurrent.
    /// </summary>
    [DebuggerDisplay("{Id}")]
    [XurrentEntity("RiskSeverity")]
    public sealed class RiskSeverity : IDataItem, IHasTranslations, INode, IRecord
    {
        /// <summary>
        ...
        {
            if (data is RiskSeverity riskSeverity)
            {
                InformationAttachmentsCollection?.Data?.AddRange(riskSeverity.InformationAttachments);
                TranslationsCollection?.Data?.AddRange(riskSeverity.Translations);
            }
        }
