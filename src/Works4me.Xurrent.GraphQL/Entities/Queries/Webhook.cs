...
{
    /// <summary>
    /// Represent a <see href="https://developer.xurrent.com/graphql/object/webhook/">Webhook</see> in Xurrent.
    /// </summary>
    [DebuggerDisplay("{Id}")]
    [XurrentEntity("Webhook")]
    public sealed class Webhook : IDataItem, INode, IRecord
    {
        /// <summary>
        ...
        /// <summary>
        /// Only trigger the webhook when the App Instance has the same App Offering reference.
        /// </summary>
        [XurrentField("appOfferingReferences")]
        public IReadOnlyList<string>? AppOfferingReferences { get; internal set; }
        ...
        {
            if (data is Webhook webhook)
            {
                DescriptionAttachmentsCollection?.Data?.AddRange(webhook.DescriptionAttachments);
            }
        }
