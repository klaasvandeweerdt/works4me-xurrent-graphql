...
{
    /// <summary>
    /// Represent a <see href="https://developer.xurrent.com/graphql/object/release/">Release</see> in Xurrent.
    /// </summary>
    [DebuggerDisplay("{Id}")]
    [XurrentEntity("Release")]
    public sealed class Release : IDataItem, IHasLifeCycleState, IHasNotes, INode, IRecord
    {
        /// <summary>
        ...
        {
            if (data is Release release)
            {
                CustomFieldsAttachmentsCollection?.Data?.AddRange(release.CustomFieldsAttachments);
                NotesCollection?.Data?.AddRange(release.Notes);
                WorkflowsCollection?.Data?.AddRange(release.Workflows);
            }
        }
