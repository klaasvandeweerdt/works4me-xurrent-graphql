...
{
    /// <summary>
    /// Represent an <see href="https://developer.xurrent.com/graphql/object/invoicecreateinput/">InvoiceCreateInput</see> in Xurrent.
    /// </summary>
    public sealed class InvoiceCreateInput : PropertyChangeSet
    {
        ...
        /// </summary>
#if NET6_0_OR_GREATER
        public InvoiceCreateInput(string supplierId, string description, DateOnly invoiceDate, string invoiceNr, decimal quantity, decimal unitPrice)
#else
        public InvoiceCreateInput(string supplierId, string description, DateTime invoiceDate, string invoiceNr, decimal quantity, decimal unitPrice)
#endif
        {
            _supplierId = Set("supplierId", supplierId);
            _description = Set("description", description);
            _invoiceDate = Set("invoiceDate", invoiceDate);
            _invoiceNr = Set("invoiceNr", invoiceNr);
            _quantity = Set("quantity", quantity);
            _unitPrice = Set("unitPrice", unitPrice);
        }
    }
}
